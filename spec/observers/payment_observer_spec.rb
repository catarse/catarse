...
RSpec.describe PaymentObserver do
  ...
  describe 'after_save' do
    ...
    context 'when pix_payment is true' do
      let(:payment) { create(:payment, payment_method: 'Pix', state: 'paid', gateway_data: {}) }
      it('should notify the contribution') do
        expect(ContributionNotification.where(template_name: 'payment_pix', user: contribution.user, contribution: contribution).count).to eq 1
      end
    end
    context "when project is not open_for_contributions" do
      let(:payment) { build(:payment, payment_method: 'Pix', state: 'paid', gateway_data: {}) }
      before do
        allow(payment.contribution.project).to receive(:open_for_contributions?).and_return(false)
        expect(payment.contribution.project).to receive(:open_for_contributions?).and_call_original()
        payment.save
      ...
      it('should not notify the contribution') do
        expect(ContributionNotification.where(template_name: 'payment_pix', user: contribution.user, contribution: contribution).count).to eq 0
      end
    end
  end

...
CREATE OR REPLACE VIEW "1"."project_contributions" AS 
 SELECT c.anonymous,
  ...
  WHERE ((is_owner_or_admin(p.user_id) OR (c.user_id = current_user_id())) 
        AND ((pa.payment_method = 'CartaoDeCredito') OR (pa.payment_method = 'BoletoBancario' 
                -- boleto payment interval
                -- boleto has less than 4 days since creation date 
                AND ( (now() - c.created_at) <= '4 days'::interval))))
	}
  end

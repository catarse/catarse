  ...
  def change
    ...
    AS $$
        ...
        BEGIN
            ...
            full_text_index :=  setweight(to_tsvector('english', unaccent(coalesce(project.name::text, ''))), 'A') || 
                                setweight(to_tsvector('english', unaccent(coalesce(project.permalink::text, ''))), 'C') || 
                                setweight(to_tsvector('english', unaccent(coalesce(project.headline::text, ''))), 'B') || 
                                setweight(to_tsvector('english', unaccent(coalesce((SELECT c.name_en FROM categories c WHERE c.id = project.category_id)::text, ''))), 'B') || 
                                setweight(to_tsvector('english', unaccent(coalesce((select array_agg(t.name)::text from public.taggings ta join public_tags t on t.id = ta.public_tag_id where ta.project_id = project.id)::text, ''))), 'B') || 
                                setweight(to_tsvector('english', unaccent(coalesce((SELECT u.name FROM users u WHERE u.id = project.user_id)::text, ''))), 'C');

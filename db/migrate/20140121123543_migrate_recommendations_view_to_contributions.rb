class MigrateRecommendationsViewToContributions < ActiveRecord::Migration
  def up
    execute "
    ...
    CREATE OR REPLACE VIEW recommendations AS
      ...
      FROM
        ...
        JOIN contributions contributions_same_projects ON p.id = contributions_same_projects.project_id
        JOIN contributions recommenders ON recommenders.user_id = contributions_same_projects.user_id
        JOIN projects recommendations ON recommendations.id = recommenders.project_id
      WHERE
        b.state = 'confirmed'
        AND contributions_same_projects.state = 'confirmed'
        AND recommenders.state = 'confirmed'
        AND b.user_id <> contributions_same_projects.user_id

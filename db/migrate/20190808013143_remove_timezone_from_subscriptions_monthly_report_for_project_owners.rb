class RemoveTimezoneFromSubscriptionsMonthlyReportForProjectOwners < ActiveRecord::Migration[4.2]
  def up
    execute <<-SQL
    ...
    CREATE OR REPLACE VIEW "public"."subscription_monthly_report_for_project_owners" AS
      ...
      GROUP BY user_address.street, user_address.complement, user_address.number, user_address.neighborhood, user_address.city, user_address.state, user_address.zipcode, proj.id, s.gateway_general_data, s.project_id, u.name, u.public_name, u.email, s.data, r.title, r.description, s.created_at, s.updated_at, u.id, pst.created_at, sub.checkout_data, fees.gateway_fee
      ORDER BY created_at;;
      ---
    SQL
  end

...
CREATE OR REPLACE VIEW "1".project_details AS 
 SELECT p.id AS project_id,
  ...
  GROUP BY posts_size.count,ct.id, p.id, c.id, u.id, c.name_en, ct.name, u.address_city, st.acronym, u.address_state, st.name, pt.progress, pt.pledged, pt.paid_pledged, pt.total_contributions, p.state, p.expires_at, pt.total_payment_service_fee, pt.total_contributors;
}
  end
end

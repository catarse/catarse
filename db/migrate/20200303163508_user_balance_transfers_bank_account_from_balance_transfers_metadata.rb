...
CREATE OR REPLACE VIEW "1"."user_balance_transfers" AS 
  SELECT bt.user_id,
      ...
      u.name AS user_name,
          ...
          END AS account_type,
          CASE
              WHEN (transferred_transition.document_type IS NULL) THEN
              CASE
                  WHEN (bank_account.account_type = 'pf'::text) THEN 'cpf'::text
                  ELSE 'cnpj'::text
              END
    ...
    FROM ((((balance_transfers bt
      LEFT JOIN LATERAL ( SELECT ba.user_id,
              ...
              bt.metadata->>'account_type' AS account_type
    ...
    SELECT bt.user_id,
       ...
       u.name AS user_name,
           ...
           CASE
               WHEN (transferred_transition.document_type IS NULL) THEN
               CASE
                   WHEN (bank_account.document_type = 'pf'::text) THEN 'cpf'::text
                   ELSE 'cnpj'::text
               END
               ELSE transferred_transition.document_type
           END AS document_type,

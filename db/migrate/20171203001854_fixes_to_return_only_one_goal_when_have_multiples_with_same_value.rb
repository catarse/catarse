class FixesToReturnOnlyOneGoalWhenHaveMultiplesWithSameValue < ActiveRecord::Migration[4.2]
  def up
    execute %Q{
CREATE OR REPLACE VIEW "1"."projects" AS
 SELECT p.id AS project_id,
    ...
    COALESCE(( SELECT
                CASE
                    WHEN (p.mode = 'sub'::text) THEN ( SELECT sum(s_1.amount) AS sum
                       FROM subscriptions s_1
                      WHERE ((s_1.project_id = p.id) AND ((s_1.status)::text = 'active'::text)))
                    ELSE ( SELECT
                            CASE
    ...
    COALESCE(( SELECT
                CASE
                    WHEN (p.mode = 'sub'::text) THEN ((( SELECT sum(s_1.amount) AS sum
                       FROM subscriptions s_1
                      WHERE ((s_1.project_id = p.id) AND ((s_1.status)::text = 'active'::text))) / COALESCE(( SELECT g.value
                       FROM goals g
                      WHERE ((g.project_id = p.id) AND (g.value >= ( SELECT sum(s_1.amount) AS sum
                               FROM subscriptions s_1
                              WHERE ((s_1.project_id = p.id) AND ((s_1.status)::text = 'active'::text))))) limit 1), ( SELECT max(goals.value) AS max
                       FROM goals
    ...
    COALESCE(( SELECT
                CASE
                    WHEN (p.mode = 'sub'::text) THEN ((( SELECT sum(s_1.amount) AS sum
                      ...
                      WHERE ((g.project_id = p.id) AND (g.value >= ( SELECT sum(s_1.amount) AS sum
                               FROM subscriptions s_1
                              WHERE ((s_1.project_id = p.id) AND ((s_1.status)::text = 'active'::text)))))), ( SELECT max(goals.value) AS max
                       FROM goals
                      WHERE (goals.project_id = p.id)))) * (100)::numeric)
                    ELSE ( SELECT pt.progress
                       FROM "1".project_totals pt

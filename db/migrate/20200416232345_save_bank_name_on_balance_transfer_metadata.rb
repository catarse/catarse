class SaveBankNameOnBalanceTransferMetadata < ActiveRecord::Migration[4.2]
  def change
    execute <<-SQL
...
AS $function$
    ...
    BEGIN
      ...
      INNER JOIN users u ON ba.user_id = NEW.user_id AND u.id = NEW.user_id
      JOIN public.banks b ON b.id = ba.bank_id LIMIT 1
      INTO bank_json;

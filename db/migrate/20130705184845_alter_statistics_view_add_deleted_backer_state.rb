class AlterStatisticsViewAddDeletedBackerState < ActiveRecord::Migration[4.2]
  def up
    execute <<-SQL
  ...
  def down
    execute <<-SQL
       ...
       FROM ( SELECT count(*) AS total_backs,
                ...
                sum(backers.value) AS total_backed
               FROM backers
              WHERE backers.state::text <> ALL (ARRAY['waiting_confirmation'::character varying::text, 'pending'::character varying::text, 'canceled'::character varying::text])) backers_totals,
        ( SELECT count(*) AS total_projects,
                count(
                    CASE
                        WHEN projects.state::text = 'successful'::text THEN 1

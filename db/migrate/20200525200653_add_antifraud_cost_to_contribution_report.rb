class AddAntifraudCostToContributionReport < ActiveRecord::Migration[4.2]
  def up
    execute <<-SQL
      ...
      CREATE VIEW public.contribution_reports_for_project_owners AS
      SELECT b.project_id,
      ...
      numbertocurrency(pa.value * p.service_fee - pa.gateway_fee - COALESCE(aa."cost", 0)) AS service_fee,
      numbertocurrency(pa.gateway_fee) AS payment_method_fee,
      numbertocurrency(COALESCE(aa."cost", 0)) AS antifraud_cost,

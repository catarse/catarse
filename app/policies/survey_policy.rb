class SurveyPolicy < ApplicationPolicy
  ...
  def permitted_attributes
    [:reward_id, survey_open_questions_attributes: [:id, :question, :description], survey_multiple_choice_questions_attributes: [:id, :question, :description, survey_question_choices_attributes: [:option, :survey_multiple_choice_question_id]], survey_address_answer: {address_attributes: [:id,:country_id, :state_id, :address_street, :address_city, :address_neighbourhood, :address_number, :address_complement, :address_zip_code, :phone_number ]}].flatten
  end

    ...
    execute <<-SQL
      ...
      SELECT
        ...
        CASE
            ...
            WHEN (COALESCE(add.address_state, '') != '') THEN  add.address_state
      ...
      JOIN
        payments pa ON pa.contribution_id = b.id
      LEFT JOIN
        rewards r ON r.id = b.reward_id
      LEFT JOIN
        shipping_fees sf ON sf.id = b.shipping_fee_id
      LEFT JOIN
        "1".surveys su ON su.contribution_id = pa.contribution_id
      LEFT JOIN
        addresses add ON add.id = b.address_id
      LEFT JOIN
        states st ON st.id = add.state_id
      LEFT JOIN
        countries country ON country.id = add.country_id
      WHERE
        pa.state = ANY(ARRAY[
          'paid'::text,
          'pending'::text,

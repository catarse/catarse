class Notification < ActiveRecord::Base
  ...
  def self.find_notification notification_type_name
    ...
    raise "There is no NotificationType with name #{notification_type_name}" if !nt
    return nt
  end

  def self.notify_backer backer, notification_type_name
    nt = find_notification notification_type_name
    create! :project => project, :backer => backer, :user => backer.user, :email_text => 'this will be removed', :text => 'this will be removed', :notification_type_id => nt.id
  end

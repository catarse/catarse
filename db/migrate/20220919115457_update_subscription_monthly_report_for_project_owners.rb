class UpdateSubscriptionMonthlyReportForProjectOwners < ActiveRecord::Migration[6.1]
  def up
    execute <<-SQL
    ...
    CREATE OR REPLACE VIEW "subscription_monthly_report_for_project_owners" AS
     SELECT s.project_id,
      ...
      GROUP BY user_address.street, user_address.phone_number, user_address.complement, user_address.number, user_address.neighborhood, user_address.city, user_address.state, user_address.zipcode, user_address.country, proj.id, s.gateway_general_data, s.project_id, u.name, u.public_name, u.email, s.data, r.title, r.description, s.created_at, s.updated_at, u.id, pst.created_at, sub.checkout_data, fees.gateway_fee, aa.cost;
    SQL
  end

class FixProjectsEndpoint < ActiveRecord::Migration
  def change
    ...
    COALESCE((SELECT CASE WHEN p.mode = 'sub' THEN
    ((SELECT sum(amount) from subscriptions s where s.project_id = p.id and status = 'active')/
      COALESCE((SELECT value from goals g where g.project_id = p.id and g.value >= ((SELECT sum(amount) from subscriptions s where s.project_id = p.id and status = 'active'))),
      (SELECT max(value) from goals where goals.project_id = p.id))
      ) * 100
    ELSE
    (( SELECT pt.progress
           FROM "1".project_totals pt
          WHERE pt.project_id = p.id))
    END), 0::numeric) AS progress,

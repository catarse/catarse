...
const paymentVM = () => {
    ...
    const populateForm = (fetchedData: UserDetails) => {
        const data = fetchedData || { address: {} };
    ...
    const isInternational = (value?: any) => {
        if (value) {
    ...
    const getSlipPaymentDate = (contribution_id: number) => {
    ...
    const fetchUser = async () => {
        const userDetails = await getUpdatedUserDetailsFromCurrentUser()

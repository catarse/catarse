class ChangeFinishedToProjectToIncludeWaitingFunds < ActiveRecord::Migration[4.2]
  def change
    execute <<-SQL
    ...
    SELECT p.id AS project_id,
      ...
      WHERE (EXISTS ( SELECT true AS bool
              ...
              WHERE (pt.state::text = ANY (ARRAY['waiting_funds'::text,'successful'::text, 'failed'::text])) AND pt.most_recent AND pt.project_id = p.id));

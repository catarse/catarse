class FixProjectFiscalInformsView < ActiveRecord::Migration[4.2]
    def up
        ...
        select project_id, user_id, mode, fiscal_year,
            ...
            min(pj.pledged_by_month) pj_pledged_by_month,
            min(pf.pledged_by_month) pf_pledged_by_month,
        ...
        left join lateral (
            select array_agg(p ORDER BY p->>'year', p->>'month') pledged_by_month from unnest(pfd.pf_pledged_by_month) p where p is not null
        ) pf on true

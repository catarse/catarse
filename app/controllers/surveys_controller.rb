  ...
  def open_questions_answer
    params[:survey_open_question_answers_attributes].each do |answer|
      ...
      question.survey_open_question_answers
        .find_or_initialize_by(id: answer['id']).update_attributes({contribution_id: answer['contribution_id'], answer: answer['answer']}) if answer['answer']
    end
  end
  ...
  def mc_questions_answer
    params[:survey_multiple_choice_question_answers_attributes].each do |answer|
      ...
      question.survey_multiple_choice_question_answers
        .find_or_initialize_by(id: answer['id']).update_attributes({contribution_id: answer['contribution_id'], survey_question_choice_id: answer['survey_question_choice_id']}) if answer['survey_question_choice_id']
    end
  end

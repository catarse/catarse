class UsePaymentMethodInsteadGatewayOnContributionReportsForProjectOwners < ActiveRecord::Migration[4.2]
  def up
    execute <<-SQL
CREATE OR REPLACE VIEW "public"."contribution_reports_for_project_owners" AS
 SELECT b.project_id,
    ...
    replace((round((pa.value * ( SELECT (settings.value)::numeric AS value
           FROM settings
          WHERE (settings.name = 'catarse_fee'::text))), 2))::text, '.'::text, ','::text) AS service_fee,

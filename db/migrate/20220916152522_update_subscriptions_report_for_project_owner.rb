class UpdateSubscriptionsReportForProjectOwner < ActiveRecord::Migration[6.1]
  def up
    execute <<-SQL
    DROP VIEW public.subscription_report_for_project_owners;
    ...
    SELECT s.project_id,
       ...
       to_char(last_paid_payment.created_at, 'DD/MM/YYYY HH24:MI:SS'::text) AS last_paid_at,
       to_char(s.created_at, 'DD/MM/YYYY HH24:MI:SS'::text) AS started_at,
     ...
     GROUP BY user_address.street, user_address.phone_number, user_address.complement, user_address.number, user_address.neighborhood, user_address.city, user_address.state, user_address.zipcode, user_address.country, s.project_id, u.name, u.public_name, u.email, last_paid_payment.checkout_data, last_payment.checkout_data, r.title, r.description, s.status, last_paid_payment.created_at, s.created_at, u.id;
    SQL
  end

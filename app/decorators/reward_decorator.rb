class RewardDecorator < Draper::Decorator
  ...
  def name
    ...
    %{
      <label data-minimum-value="#{source.minimum_value > 0 ? source.minimum_value : '10'}" class="w-form-label fontsize-base fontweight-semibold u-marginbottom-10" for="contribution_reward#{source.id && "_#{source.id}"}">#{source.minimum_value > 0 ? "#{source.display_minimum}  #{I18n.t('rewards.index.or_more')}" : I18n.t('rewards.index.dont_want')}</label>
        ...
        <div class="w-col w-col-8 w-col-small-8 w-col-tiny-8 w-sub-col-middle w-clearfix">
          <div class="w-row">
            ...
            <div class="w-col w-col-9 w-col-small-9 w-col-tiny-9">
              <input class="user-reward-value back-reward-input-reward" type="tel" min="#{source.minimum_value}" placeholder="#{source.minimum_value > 0 ? source.minimum_value : '10'}">
            </div>
          </div>
